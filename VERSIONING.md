# Versioning — ask-rails-harness-mcp
...
Patch numbers advance by exactly one per release: `0.2.7` → `0.2.8` →
`0.2.9`. Never skip or jump patch numbers (`0.2.7` → `0.2.10` is wrong), even
...
gemchain guard ask-rails-harness-mcp
gemchain update ask-rails-harness-mcp <new-version> --dry-run
gemchain update ask-rails-harness-mcp <new-version>
```
...
3. Build passes — `bundle exec rake build` (or
   `gem build ask-rails-harness-mcp.gemspec`).
4. Changelog — `Unreleased` entries moved under the new `X.Y.Z` heading with
   the release date.
5. Version — bumped in `lib/ask/rails/mcp/version.rb` by gemchain, not by
   hand.
...
| `lib/ask/rails/mcp/version.rb` at HEAD | `X.Y.Z` |
| Git tag | `vX.Y.Z` |
...
- Bug fix only: `0.2.7` → `0.2.8`.
- Several bug fixes ship together: one release, `0.2.7` → `0.2.8` (never
  `0.2.10`).
- New backwards-compatible API: `0.2.8` → `0.3.0`.
- Breaking change while pre-1.0: `0.3.0` → `0.4.0`, with a `Changed` /
  `Removed` changelog entry calling out the break.
- First stable release: `0.9.x` → `1.0.0`.
- This gem bumped because a dependency released: patch only, e.g. `0.2.8` →
  `0.2.9`, via the gemchain cascade — never hand-published.
